...
class StableDiffusion(nn.Module):
    ...
    def invert_noise(self, latents, target_t, text_embeddings, guidance_scale=-7.5, n_steps=10, eta=0.3):
        ...
        for i in range(len(timesteps)-1):
            ...
            sigma_t = eta * torch.sqrt((1.0 - alpha_bar_t) / (1.0 - alpha_bar_t_next)) * torch.sqrt(1.0 - alpha_bar_t_next / alpha_bar_t)
            sigma_t = torch.clamp(sigma_t, min=0.0, max=1.0)
            ...
            x0_pred = (noisy_latents - torch.sqrt(1 - alpha_bar_t) * noise_pred) / torch.sqrt(alpha_bar_t)
            noisy_latents = alpha_bar_t_next.sqrt() * (x0_pred + (((1-alpha_bar_t)/alpha_bar_t).sqrt()+(1/(alpha_bar_t_next)-1).sqrt()-(1/alpha_bar_t-1).sqrt()) * noise_pred)
